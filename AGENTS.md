...
- 攻略情報を収集する際は、1つの攻略サイトに偏らない。公式・ゲーム内告知、大手攻略サイト複数、GameToraなどのDB、noteや個人ブログなどの実践メモを必要に応じて横断し、内容を照合する。
- noteや個人ブログを参照する場合は、主観・検証環境・投稿/更新日の影響が大きいことを前提にし、公式情報や複数攻略サイトと矛盾しない範囲で補助情報として扱う。
- 情報源欄では確認内容と確認日を残し、古い記事を使う場合は「基礎方針の照合」「継承固有の考え方」など用途を限定して note に書く。
...
- SNS共有リンクは `_includes/share-links.html` を使い、`_includes/floating-menu.html` の右下固定メニュー内に共有ボタンを自動挿入し、各layout末尾に共有ダイアログを1つだけ挿入する。本文へ重複して書かない。
- 共有URLは `page.url | absolute_url`、共有文は `page.title | default: site.title`。共有先はX、mixi2、LINE、Facebook、ページURLを基本にする。
...
- 色・影などの共通デザイントークンは `assets/css/tokens.css` の `:root` に集約する。`home.css` / `guide.css` / `list.css` はこの変数を前提にし、原則として同名の `:root` 変数を重複定義しない。
- `_includes/head.html` では `assets/css/tokens.css` を各ページ固有CSSより前に読み込む。ファイル名はJekyllで公開される通常名にし、先頭アンダースコア付きの `_tokens.css` などにはしない。
- `tokens.css` が読み込めないと `var(--red)` などが未定義になり、ボタン・カード・見出しの色が抜ける。色抜けが起きた場合は、生成HTMLのCSS読み込み順、`/assets/css/tokens.css` の404有無、ブラウザキャッシュ、`getComputedStyle(document.documentElement).getPropertyValue('--red')` を確認する。
- 同種の障害が再発し、原因が `tokens.css` の読み込み分離にあると判断した場合は、`home.css` と `guide.css` の先頭に `:root` 変数を戻す自己完結方式へ戻すことを検討する。
...
- 生成HTMLで `/assets/css/tokens.css` がページ固有CSSより前に読み込まれ、ブラウザで `--red` などのCSS変数が空になっていないこと
